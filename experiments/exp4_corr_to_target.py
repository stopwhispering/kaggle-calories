...
a = 1
# for each col, compute the correlation with the target
for col in df_train.columns:
    print(f"{col=}, {df_train[col].corr(ser_targets_train):.5f}")

ser_targets_original = ser_targets_train.copy()
duration_target_factor = (ser_targets_train / df_train['Duration']).mean()
ser_targets_train_expected_from_duration = df_train['Duration'] * duration_target_factor
ser_targets_diff = ser_targets_train - ser_targets_train_expected_from_duration
# ser_targets_train = ser_targets_train * duration_target_factor
