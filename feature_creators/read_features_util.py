...
include_original=False,
    ...
    file_paths = [
        f for f in path.glob("*metadata.pkl")
    ]
    ...
    if column_names is not None:
        ...
        assert len(metadata) == len(column_names), f"Missing metadata for columns: {set(column_names) - {m['column_name'] for m in metadata}}"
...
def read_features(
    ...
    metadata: list[dict] = _read_all_metadata(column_names=column_names, include_original=include_original, type_=type_)
    ...
    for m in metadata:
        ...
        if (path / f"{m['filename_prefix']}_train.parquet").exists():
            # read parquet files
            ser_train = pd.read_parquet(path / f"{m['filename_prefix']}_train.parquet").iloc[:, 0]
            ser_test = pd.read_parquet(path / f"{m['filename_prefix']}_test.parquet").iloc[:, 0]
        else:
            # read pickle files
            ...
            ser_train.to_frame().to_parquet(path / f"{m['filename_prefix']}_train.parquet")
            ser_test.to_frame().to_parquet(path / f"{m['filename_prefix']}_test.parquet")
            logging.warning(f"Saved {m['filename_prefix']}_train.parquet and {m['filename_prefix']}_test.parquet")
