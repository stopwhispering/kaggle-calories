...
from calories.constants import (
    ...
    PATH_FEATURES_BY_FOLD_WITH_ORIGINAL,
)
...
def _read_all_metadata(
    ...
    if not include_original:
        file_paths = [f for f in PATH_FEATURES_BY_FOLD.glob("*metadata.pkl")]
    else:
        file_paths = [
    ...
    if feature_lists is not None:
        ...
        missing_feature_lists = [
            f
            for f in feature_lists
            if f not in [set(m["feature_list"]) for m in metadata]
        ]
        assert not missing_feature_lists, (
            f"Missing feature lists: {missing_feature_lists}"
        )

        metadata = [m for m in metadata if m["agg"] == agg]  # todo other aggs
    # assert same outer_fold
    assert all(m["outer_folds"] == metadata[0]["outer_folds"] for m in metadata), (
    ...
    for i_outer_fold in tqdm(range(m["outer_folds"])):
        ...
        for m in metadata:
            if (
                from_parquet
                and (
                    path / f"{m['filename_prefix']}_train_{i_outer_fold}.parquet"
                ).exists()
            ):
                ser_train: pd.Series = pd.read_parquet(
                    path / f"{m['filename_prefix']}_train_{i_outer_fold}.parquet"
                ).iloc[:, 0]
                ser_val: pd.Series = pd.read_parquet(
                    path / f"{m['filename_prefix']}_val_{i_outer_fold}.parquet"
                ).iloc[:, 0]
                try:
                    ser_test: pd.Series = pd.read_parquet(
                        path / f"{m['filename_prefix']}_test_{i_outer_fold}.parquet"
                    ).iloc[:, 0]
                except:
                    print(
                        f"WARNING: {m['filename_prefix']}_test_{i_outer_fold}.parquet not found"
                    )
                    raise
            else:
                ser_train: pd.Series = pd.read_pickle(
                    path / f"{m['filename_prefix']}_train_{i_outer_fold}.pkl"
                )
                ser_val: pd.Series = pd.read_pickle(
                    path / f"{m['filename_prefix']}_val_{i_outer_fold}.pkl"
                )
                ser_test: pd.Series = pd.read_pickle(
                    path / f"{m['filename_prefix']}_test_{i_outer_fold}.pkl"
                )
        ...
        assert all(
            (ser_train.index == train_series[0].index).all()
            for ser_train in train_series
