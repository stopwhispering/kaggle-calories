...
def save(
    ...
    metadata = {
        ...
        "type": "map original targets min",
        "subtype": "map original targets min",
        "description": f"map original targets min",
...
if __name__ == "__main__":
    ...
    for feature_combination in tqdm(potential_combinations[::-1]):
        ...
        colname = f"map_orig_targets_min_{'_'.join(feature_combination)}"

        ser_original_concat = df_original[feature_combination].astype(str).agg("_".join, axis=1)
        ser_original_min = df_original.groupby(ser_original_concat)["Calories"].min()
        ...
        ser_map_orig_train = ser_train_concat.map(ser_original_min).rename(colname).astype("float32")
        ser_map_orig_test = ser_test_concat.map(ser_original_min).rename(colname).astype("float32")
