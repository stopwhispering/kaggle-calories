...
if __name__ == "__main__":
    ...
    for feature_list in tqdm(combinations):
        ...
        ser_te_test = te.transform(df_test_current).astype(DTYPE)

        ser_te_train.name = f"{ser_te_train.name}_not_folded_no_fillna"
        # ser_te_val.name = f"{ser_te_val.name}_no_fillna"
        ser_te_test.name = f"{ser_te_test.name}_not_folded_no_fillna"
        ...
        metadata = {
            ...
            "type": "te",
            "subtype": "te_mean not folded without fillna",
            "description": f"Target encoding of {feature_list} with mean aggregation, not folded, {N_INNER_FOLDS=} without fillna",
