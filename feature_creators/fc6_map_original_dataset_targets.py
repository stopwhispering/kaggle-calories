...
def save(
    ...
    metadata = {
        ...
        "type": "map original targets",
        "subtype": "map original targets",
        "description": f"map original targets",
...
if __name__ == "__main__":
    ...
    for feature_combination in tqdm(potential_combinations[::-1]):
        ...
        colname = f"map_orig_targets_{'_'.join(feature_combination)}"

        ser_original_concat = df_original[feature_combination].astype(str).agg("_".join, axis=1)
        ser_original_mean = df_original.groupby(ser_original_concat)["Calories"].mean()
        ...
        ser_map_orig_train = ser_train_concat.map(ser_original_mean).rename(colname).astype("float32")
        ser_map_orig_test = ser_test_concat.map(ser_original_mean).rename(colname).astype("float32")
