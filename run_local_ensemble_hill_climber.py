...
from calories.constants import PATH_TRAIN, PATH_PREDS_FOR_ENSEMBLES, PATH_ENSEMBLE_SUBMISSIONS
from my_ml_util.ensembling.hill_climbing import HillClimber, score_multiple_rmse, \
...
test_files_from_full = [file for file in PATH_PREDS_FOR_ENSEMBLES.glob("*_test_from_full.pkl")]
...
df_submission = ser_test_ensemble.reset_index().rename(
    columns={"pred": "Calories"}
)
df_submission['Calories'] = np.clip(df_submission['Calories'], 0, None)
df_submission.to_csv(PATH_ENSEMBLE_SUBMISSIONS / filename, index=False)

df_weights.to_csv(PATH_ENSEMBLE_SUBMISSIONS / f"hc_weights_{date_time_str}.csv", index=False)

ser_ensemble.name = 'Calories'
df_ensemble_oof = ser_ensemble.reset_index().rename(
    columns={"pred": "Calories"}
)
df_ensemble_oof['Calories'] = np.clip(df_ensemble_oof['Calories'], 0, None)
df_ensemble_oof.to_csv(PATH_ENSEMBLE_SUBMISSIONS / f"hc_train_ensemble_{date_time_str}.csv", index=False)
