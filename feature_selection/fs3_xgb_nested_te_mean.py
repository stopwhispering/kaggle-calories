...
# )
nested_features_column_names = nested_features_by_outer_fold[0][
    0].columns.tolist()
print(f"{len(nested_features_by_outer_fold)=}. Total duration so far: {time.time() - time_start:.2f} seconds")
...
INITIAL_FEATURES_TO_DISCARD = [
    ...
    'te_MEAN_Sex_Duration_Heart_Rate_Body_Temp_no_fillna', 'te_MEAN_Age_Duration_Heart_Rate_no_fillna', 'te_MEAN_Sex_Age_Duration_Heart_Rate_no_fillna',
    'te_MEAN_Height_Body_Temp_no_fillna', 'te_MEAN_Weight_Body_Temp_no_fillna', 'te_MEAN_Height_Weight_Body_Temp_no_fillna',
]
print(f'{len(INITIAL_FEATURES_TO_DISCARD)=}')


def add_target_encoding(df_train, df_val, df_test, ser_targets_train, i_fold: int):
    selected_columns = [c for c in nested_features_column_names if c in df_train.columns]
