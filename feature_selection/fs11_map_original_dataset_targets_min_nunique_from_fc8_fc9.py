...
df_train_features, _df_test_features = feature_store.read_features(
    criteria={"type": ("map original targets min",
                       "map original targets nunique")}  # fc8+fc9
)
...
def score_dataset(
    ...
    params_catboost = {
        "loss_function": "RMSE",
        "learning_rate": 0.1,  # 0.20 -> ca. 12 secs
    }
...
df_results.to_excel(PATH_CACHE / "df_results_fs11.xlsx", index=False)
