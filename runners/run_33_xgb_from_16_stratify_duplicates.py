...
df_duplicates["unique_no"] = df_duplicates.groupby(list(df_duplicates.columns)).ngroup() + df_no_duplicates["unique_no"].max() + 1

# now we can merge the two dataframes
df_train_for_strat = pd.concat([df_no_duplicates, df_duplicates], axis=0).sort_index().drop('Calories', axis=1)

# instead of default kfold, we use stratified kfold
# splits = KFold(n_splits=5, shuffle=True, random_state=42).split(df_train)
splits = StratifiedKFold(n_splits=5, shuffle=True, random_state=42).split(df_train_for_strat, df_train_for_strat["unique_no"])
