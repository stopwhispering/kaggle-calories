...
import numpy as np
from calories.constants import PATH_TRAIN, PATH_TEST, PATH_PREDS_FOR_ENSEMBLES, PATH_ORIGINAL
...
print(f'Before Mapping Original: {df_train.shape=}, {df_test.shape=}')
numerical_features = df_train.select_dtypes('number').columns.tolist()
mean_value_original = df_original['Calories'].mean()
print(f'{mean_value_original=}')
for feature in numerical_features:
    feature_2 = f"{feature}_2"
    df_train[feature_2] = df_train[feature].map(df_original.groupby(feature)['Calories'].mean())
    df_train[feature_2] = df_train[feature_2].fillna(mean_value_original)
    df_test[feature_2] = df_test[feature].map(df_original.groupby(feature)['Calories'].mean() )
    df_test[feature_2] = df_test[feature_2].fillna(mean_value_original)
print(f'After Mapping Original: {df_train.shape=}, {df_test.shape=}')
