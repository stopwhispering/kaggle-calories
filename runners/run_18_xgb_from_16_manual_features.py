...
df_train["Exercise_Duration_Category"] = pd.cut(
    df_train["Duration"], bins=[0, 30, 60, 180], labels=["Short", "Medium", "Long"]
)
df_test["Exercise_Duration_Category"] = pd.cut(
    df_test["Duration"], bins=[0, 30, 60, 180], labels=["Short", "Medium", "Long"]
)
df_train["Age_Group"] = pd.cut(
    df_train["Age"], bins=[0, 30, 50, 100], labels=["Young", "Middle-Aged", "Old"]
)
df_test["Age_Group"] = pd.cut(
    df_test["Age"], bins=[0, 30, 50, 100], labels=["Young", "Middle-Aged", "Old"]
)
