...
df_orig = pd.read_csv(PATH_ORIGINAL).rename({'Gender': 'Sex'}, axis=1)


class FrequencyEncoder:
    # def __init__(self, df_train_original, df_test_original, columns):
    def __init__(self, df_train_original, columns):
        self.columns = columns
        self.df_cat_combined = df_orig[columns].copy()
        # self.df_cat_combined = pd.concat([df_train_original[columns], df_test_original[columns]],
        #                                  axis=0,
        #                                  ignore_index=True)

    def encode(self, df: pd.DataFrame) -> pd.DataFrame:
        """returns the new columns as a DataFrame, does not alter the provided DataFrame; provide train, val, or test"""
        df_new = pd.DataFrame(index=df.index)
        for col in self.columns:
            ser_frequency_map = self.df_cat_combined[col].value_counts(dropna=False).to_dict()
            try:
                ser_frequencies = df[col].map(ser_frequency_map).astype("int")
            except:
                ser_frequencies = df[col].map(ser_frequency_map).astype("float32")  # int does not accept NaN
            df_new['freq_' + col] = ser_frequencies

        return df_new


frequency_encoder = FrequencyEncoder(df_orig, df_train.columns.tolist())
df_train_freq = frequency_encoder.encode(df_train)
df_train = pd.concat([df_train, df_train_freq], axis=1)

df_test_freq = frequency_encoder.encode(df_test)
df_test = pd.concat([df_test, df_test_freq], axis=1)
...
df_train_features, df_test_features = feature_store.read_features(
    column_names=['Combine_Sex_Duration', 'Multiply_Weight_Duration', 'Plus_Age_Duration',
